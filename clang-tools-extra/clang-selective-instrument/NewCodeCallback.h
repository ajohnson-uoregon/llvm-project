...
DeclarationMatcher insert_prematch =
  functionDecl(matchesName("insert_prematch*"),
    hasBody(compoundStmt(allOf(
      hasAnySubstatement(
        declStmt(containsDeclaration(0,
          varDecl(hasName("matchers")).bind("matcher_list")
        ))
      ),
      hasAnySubstatement(
        cxxForRangeStmt(allOf(
          hasRangeInit(ignoringParenImpCasts(declRefExpr(to(
            varDecl(hasName("matchers"))
          )))),
...
DeclarationMatcher insert_postmatch =
  functionDecl(matchesName("insert_postmatch*"),
    hasBody(compoundStmt(allOf(
      hasAnySubstatement(
        declStmt(containsDeclaration(0,
          varDecl(hasName("matchers")).bind("matcher_list")
...
DeclarationMatcher replace_match =
  functionDecl(matchesName("replace*"),
    hasBody(compoundStmt(allOf(
      hasAnySubstatement(
        declStmt(containsDeclaration(0,
          varDecl(hasName("matchers")).bind("matcher_list")
...
public:
  ...
  void run(const MatchFinder::MatchResult& result) override {
    ...
    if (!matcher_list || !context->getSourceManager().isWrittenInMainFile(matcher_list->getBeginLoc())) {
      return;
    }
    ...
    const CompoundStmt* new_code = result.Nodes.getNodeAs<CompoundStmt>("new_code");
    if (!new_code || !context->getSourceManager().isWrittenInMainFile(new_code->getBeginLoc())) {
      return;
    }
    ...
    if (!new_code->body_empty()) {
      ...
      SourceLocation eol = Lexer::getLocForEndOfToken(new_code->body_back()->getBeginLoc(), 0, context->getSourceManager(), context->getLangOpts());
    ...
    llvm::Optional<llvm::MemoryBufferRef> buff = context->getSourceManager().getBufferOrNone(fid);

    char* code = new char[end_offset - begin_offset + 1];
    if (buff.hasValue()) {
      // printf("buffer?????\n%s\n", &(buff->getBufferStart()[begin_offset]));
      memcpy(code, &(buff->getBufferStart()[begin_offset]), (end_offset-begin_offset+1)*sizeof(char));
      code[end_offset-begin_offset] = '\0'; // force null terminated for Reasons
      printf("code??? %s\n", code);
    }
    else {
      printf("no buffer :<\n");
